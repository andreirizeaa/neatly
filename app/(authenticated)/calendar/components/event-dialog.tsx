    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4">
          ...
          {/* Show source info for deadline-sourced events */}
          {event?.sourceType === 'deadline' && event?.sourceEvidence && (
            <div className="rounded-lg border bg-muted/50 p-3 space-y-2">
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center rounded-md bg-amber-100 px-2 py-1 text-xs font-medium text-amber-800 dark:bg-amber-900/30 dark:text-amber-400">
                  From Email Analysis
                </span>
                {event?.threadId && (
                  <a
                    href={`/analysis/${event.threadId}`}
